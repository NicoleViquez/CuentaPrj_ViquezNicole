...
public class PrincipalCuenta {
    public static void main(String[] args) {
        ...
        while (!salirDelMenu) {
            ...
            System.out.println("9) Consultar Estado");
            ...
            switch (opcion) {
                case "1": {
                    System.out.println("Seleccione el tipo de creación:");
                    System.out.println("1) Crear cuenta con nombre y saldo");
                    System.out.println("2) Crear cuenta solo con saldo");
                    System.out.print("Opción: ");
                    String tipoCreacion = scanner.nextLine().trim();
                    if (tipoCreacion.equals("1")) {
                        ...
                        Cuenta cuentaNueva = new Cuenta(saldoInicial);
                        listaCuentas.add(cuentaNueva);
                        indiceCuentaActual = listaCuentas.size() - 1;
                        System.out.println("Cuenta creada correctamente con índice " + indiceCuentaActual + " y código: " + cuentaNueva.getCodCuenta());
                        System.out.println("La cuenta fue creada sin nombre. Puede asignarlo luego con la opción correspondiente.");
                    } 
                    else {
                        System.out.println("Opción de creación inválida.");
                    }
                ...
                case "5": {
                    ...
                    listaCuentas.get(indiceCuentaActual).setNombreCuentaHabiente(nombreNuevo);
                ...
                case "7": {
                    ...
                    try {
                        ...
                        Cuenta cuentaSeleccionada = listaCuentas.get(indiceCuentaActual);
                        double saldoAntes = cuentaSeleccionada.getSaldo();
                        double saldoDespues = cuentaSeleccionada.retirar(montoRetiro);
                        if (saldoDespues < saldoAntes) {
                            System.out.printf("Retiro realizado. Saldo actual: %.2f%n", saldoDespues);
                        } else {
                            System.out.printf("Fondos insuficientes. Saldo actual: %.2f%n", saldoDespues);
                        }
                    } catch (NumberFormatException e) {
                        System.out.println("Monto inválido.");
                    }
